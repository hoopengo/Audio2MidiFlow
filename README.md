# 🎵 Audio2MidiFlow

## 📋 Что это за проект?

Audio2MidiFlow - это веб-приложение, которое позволяет загружать аудиофайлы (например, MP3) и конвертировать их в MIDI формат. Проект состоит из:

- **Backend** - API на Python с использованием FastAPI
- **Frontend** - веб-интерфейс на Next.js (React)

## 🛠️ Требования для запуска

Перед началом убедись, что у тебя установлены:

### Обязательные:

- **Python 3.12+** - для бэкенда
- **Node.js 22+** - для фронтенда
- **npm** - менеджер пакетов Node.js

### Опционально (для Docker-запуска):

- **Docker** - контейнеризация
- **Docker Compose** - управление контейнерами

## 🚀 Способы запуска

Есть два способа запустить проект:

### Способ 1: Автоматическая настройка (рекомендуется)

Самый простой способ - использовать готовый скрипт настройки:

```bash
# Клонируй репозиторий (если ещё не сделал этого)
git clone https://github.com/hoopengo/Audio2MidiFlow.git
cd Audio2MidiFlow

# Запусти скрипт настройки
chmod +x setup.sh
./setup.sh
```

Скрипт автоматически:

- Проверит наличие Python и Node.js
- Создаст виртуальное окружение для Python
- Установит все зависимости
- Создаст необходимые директории
- Настроит переменные окружения

После завершения настройки переходи к разделу "Запуск серверов".

### Способ 2: Ручная настройка

#### Настройка бэкенда:

```bash
# Перейди в директорию бэкенда
cd backend

# Создай виртуальное окружение
python3.12 -m venv .venv

# Активируй его
source .venv/bin/activate  # для Linux/Mac
# или
.venv\Scripts\activate     # для Windows

# Установи зависимости
pip install --upgrade pip
pip install -r requirements.txt

# Создай необходимые директории
mkdir -p uploads outputs data

# Создай файл .env (если его нет)
cat > .env << EOF
DEBUG=true
LOG_LEVEL=INFO
# SQLite database stored in /app/data inside the backend container (mounted from ./data on host)
DATABASE_URL=sqlite:////app/data/audio2midi.db
UPLOAD_DIR=./uploads
OUTPUT_DIR=./outputs
MAX_FILE_SIZE=52428800
CORS_ORIGINS=["http://localhost:3000"]
EOF
```

#### Настройка фронтенда:

```bash
# Перейди в директорию фронтенда
cd frontend

# Установи зависимости
npm install

# Cоздать .env.local
cat > .env.local << EOF
NEXT_PUBLIC_API_URL=http://localhost:8000/api/v1
NEXT_PUBLIC_DEV_MODE=true
EOF
```

## 🖥️ Запуск серверов

После настройки проекта нужно запустить оба сервера. Открой два терминала:

### Терминал 1 - Запуск бэкенда:

```bash
# Активируй виртуальное окружение
cd backend
source .venv/bin/activate  # для Linux/Mac
# или
.venv\Scripts\activate     # для Windows

# Запусти сервер
python -m app.main
```

Бэкенд будет доступен по адресу: http://localhost:8000

### Терминал 2 - Запуск фронтенда:

```bash
# Перейди в директорию фронтенда
cd frontend

# Запусти сервер разработки
npm run dev
```

Фронтенд будет доступен по адресу: http://localhost:3000

## 🐳 Запуск через Docker (альтернативный способ)

```bash
# Собери и запусти контейнеры
docker-compose up --build
```

Это запустит и бэкенд, и фронтенд в контейнерах. Бэкенд будет на http://localhost:8000, фронтенд на http://localhost:3000.

После запуска:

- **Веб-интерфейс**: http://localhost:3000
- **API документация**: http://localhost:8000/docs
- **Health check**: http://localhost:8000/health

## 📝 Переменные окружения

### Backend (.env):

- `DEBUG` - режим отладки (true/false)
- `LOG_LEVEL` - уровень логирования (INFO, DEBUG, ERROR)
- `DATABASE_URL` - URL базы данных
- `UPLOAD_DIR` - директория для загрузок
- `OUTPUT_DIR` - директория для результатов
- `MAX_FILE_SIZE` - максимальный размер файла в байтах

### Frontend (.env.local):

- `NEXT_PUBLIC_API_URL` - URL API бэкенда
- `NEXT_PUBLIC_DEV_MODE` - режим разработки
